...
class MY_Model extends CI_Model
{
    ...
    public function get_some($feild='*',$where='',$row=FALSE)
    {
      $this->_database->select($feild);
      if (is_array($where)) {
        $this->db->where($where);
      }
      if ($row==TRUE) {
      ...
      return $this->get_all();
        // return $this->_database->get($this->_table)->result();
    }
    ...
    {
      return $this->get_json($this->get_all());
    }

    public function setview($value='')
    {
      $this->_table = $value ?: 'v_'.$this->_table;
    }

    public function get_page($per=10,$offset=0)
    {
      $offset = $offset * $per ?: 0;
      ...
      $next_page = ($offset + $per);

      $data['total_page'] = $total_pages;

      $page_active = ($offset / $per) + 1;
      ...
      $data['prev_page'] = $next_page - $per-$per;
      $data['active_page'] = $page_active;
      $data['item_page'] = range(1, $total_pages);
